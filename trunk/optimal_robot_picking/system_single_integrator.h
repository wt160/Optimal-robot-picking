...
namespace SingleIntegrator {
    ...
    public:    
         ...
         */
        ~Trajectory ();
		Trajectory(double costFromParent, const State& s);
        /*!
    ...
    public:    
         ...
         * More elaborate description
         */
        std::list<Polygon_2*> obstacles;
        
		std::map<std::pair<int, int>, std::set<Polygon_2*>> obs_hash_map;
		//int * hash_map_size;
        /*!
		...
		void updateHashMap(Polygon_2*);
		std::set<std::pair<int, int>> getIndexForHash(Polygon_2 ob, double unit_size);
        /*!
        ...
        bool isLineInCollision(double *start, double *end);
        /*!
		...
		/*
		 * \brief Return a sample state in a given region
		 *
		 */
		int sampleStateInRegion(State &randomStateOut, Polygon_2* sampleRegion);
